...
function Navbar() {
  ...
  function chajaWrite() {
    if(window.innerWidth < 640) {
      ...
      // document.getElementById("chajainput").classList.toggle("")
  ...
  return(
    ...
    <nav style={{height: '107px'}} id="navBar" className="p-0 flex justify-left border-t-2 border-gray-300 fixed -bottom-0 w-screen bg-white">
      ...
      <form onSubmit={newChaja} className="static z-20 self-center flex mx-2 sm:mx-10 pr-4">
      ...
      <article id="navbuttons" className="absolute -right-0 bottom-8 z-0 flex text-purple-700 font-bold text-right items-center nowrap mr-10 ">
        <p className="pl-4 md:px-4 pt-2 hover:transform hover:-rotate-12 duration-75"><Link to='/login'>LOGIN</Link></p>
        <p className="pl-4 md:px-4 pt-2 hover:transform hover:rotate-12 duration-75"><Link to='/about'>CHAJÁ???</Link></p>
      </article>
    </nav>
  )
